...
const AGENT_COLORS: Record<string, string> = {
  commercial_strategy_analyst: "#60A5FA",
  pricing_analyst: "#F4B740",
  risk_underwriting_analyst: "#EF6A6A",
  tenant_synthesizer: "#C9A66B",
};

const AGENT_LABELS: Record<string, string> = {
  commercial_strategy_analyst: "Commercial Strategy Analyst",
  pricing_analyst: "Pricing Analyst",
  risk_underwriting_analyst: "Risk Underwriting Analyst",
  tenant_synthesizer: "Tenant Synthesizer",
};

export function TenantDebateStream({ output, presetId }: { output: M8Output; presetId?: string }) {
  const [turns, setTurns] = useState<Turn[]>([]);
  const [synthesis, setSynthesis] = useState<TenantSynthesis | null>(null);
  ...
  const handle = (evt: AgentEvent) => {
    if (evt.event === "phase") {
      ...
      const data = evt.data as TenantSynthesis | { ok?: boolean };
  ...
  const run = async () => {
    ...
    try {
      for await (const evt of callAgent("tenant-optimize", {
        input: output.input,
        sellableMW: output.sellableMW,
        unallocatedMW: output.unallocatedMW,
        weightedPriceUSDPerMWh: output.weightedPriceUSDPerMWh,
        totalProjectedRevenueUSDm: output.totalProjectedRevenueUSDm,
        weightedGrossMarginPct: output.weightedGrossMarginPct,
        paybackMonthsP50: output.paybackMonthsP50,
        paybackMonthsP90: output.paybackMonthsP90,
        tenantFits: output.tenantFits,
        revenueScenarios: output.revenueScenarios,
        preset_id: presetId,
      })) {
        handle(evt);
      }
      ...
      <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 2 }}>
        <Box>
          <Typography variant="overline" color="text.secondary">
            Tenant optimization debate
          </Typography>
          <Typography variant="body2" color="text.secondary">
            Commercial + Pricing + Underwriting analysts (Sonnet) → Tenant Synthesizer (Opus).
          </Typography>
        </Box>
