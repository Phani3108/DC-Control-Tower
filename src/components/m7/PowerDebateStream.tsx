...
const AGENT_COLORS: Record<string, string> = {
  grid_operations_analyst: "#60A5FA",
  market_dispatch_analyst: "#F4B740",
  resiliency_analyst: "#5CC7B2",
  power_synthesizer: "#C9A66B",
};

const AGENT_LABELS: Record<string, string> = {
  grid_operations_analyst: "Grid Ops Analyst",
  market_dispatch_analyst: "Market Dispatch Analyst",
  resiliency_analyst: "Resiliency Analyst",
  power_synthesizer: "Power Synthesizer",
};

export function PowerDebateStream({ output, presetId }: { output: M7Output; presetId?: string }) {
  const [turns, setTurns] = useState<Turn[]>([]);
  const [synthesis, setSynthesis] = useState<PowerSynthesis | null>(null);
  ...
  const handle = (evt: AgentEvent) => {
    if (evt.event === "phase") {
      ...
      const data = evt.data as PowerSynthesis | { ok?: boolean };
  ...
  const run = async () => {
    ...
    try {
      for await (const evt of callAgent("power-balance", {
        input: output.input,
        grossFacilityMW: output.grossFacilityMW,
        firmAvailableMW: output.firmAvailableMW,
        reserveHeadroomMW: output.reserveHeadroomMW,
        curtailedLoadMW: output.curtailedLoadMW,
        batteryRuntimeMinAtDeficit: output.batteryRuntimeMinAtDeficit,
        blendedPowerCostUSDPerMWh: output.blendedPowerCostUSDPerMWh,
        annualPowerCostUSDm: output.annualPowerCostUSDm,
        dispatchPlan: output.dispatchPlan,
        gridRisks: output.gridRisks,
        preset_id: presetId,
      })) {
        handle(evt);
      }
      ...
      <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 2 }}>
        <Box>
          <Typography variant="overline" color="text.secondary">
            Power balancing debate
          </Typography>
          <Typography variant="body2" color="text.secondary">
            Grid Ops + Market Dispatch + Resiliency analysts (Sonnet) → Power Synthesizer (Opus).
          </Typography>
        </Box>
